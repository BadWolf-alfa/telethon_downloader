...
class TelegramBot:
    def __init__(self):
        ...
        self.group_scanner = GroupScanner(self)
        self.auto_scanner = AutoScanner(self)
    ...
    async def handle_new_message(self, event):
        try:
            ...
            # Procesar comandos de usuarios autorizados
            if AUTHORIZED_USER and (event.message.message).startswith("/rename"):
                ...
                # Descarga normal para usuarios autorizados
                await self.download_media_with_retries(event.message)
            
            else:
                # Verificar si debe procesarse por auto scanner
                # (independientemente de si el usuario está autorizado)
                if self.auto_scanner.is_auto_scan_enabled():
                    auto_processed = await self.auto_scanner.process_auto_download(event.message)
                    if auto_processed:
                        logger.logger.info(f"Message processed by auto scanner")
                        
        except Exception as e:
            logger.logger.error(f"handle_new_message Exception: {e}")
            # Solo responder con error si es un usuario autorizado
            if hasattr(event, 'reply') and self.AUTHORIZED_USER(event.message):
                await event.reply(f"Exception in handle new message: {e}")
    ...
    def printEnvironment(self):
        ...
        # Group Scanner attributes
        self.printAttribute("SCAN_MIN_FILE_SIZE_MB")
        self.printAttribute("SCAN_MAX_PARALLEL")
        self.printAttribute("SCAN_DEFAULT_DAYS")
        self.printAttribute("SCAN_MAX_DAYS")
        self.printAttribute("SCAN_DEFAULT_LIMIT")
        
        # Auto Scanner attributes
        self.printAttribute("AUTO_SCAN_ENABLED")
        self.printAttribute("AUTO_SCAN_GROUPS")
        self.printAttribute("AUTO_SCAN_MIN_SIZE_MB")
        self.printAttribute("AUTO_SCAN_NOTIFY_USER")
        self.printAttribute("AUTO_SCAN_WHITELIST_ONLY")
    ...
    async def commands(self, message):
        try:
            ...
            if self.AUTHORIZED_USER(message):
                ...
                # Verificar si es un comando de escaneo de archivos grandes
                if isinstance(process_command, dict) and 'action' in process_command:
                    if process_command['action'] in ['scan_large', 'scan_list']:
                        # Ejecutar el escaneo de forma asíncrona
                        asyncio.create_task(
                            self.group_scanner.scan_and_download_group(
                                group_id=process_command['group_id'],
                                days_back=process_command['days_back'],
                                limit=process_command['limit'],
                                auto_download=process_command['auto_download']
                            )
                        )
                        
                        # Enviar confirmación inmediata
                        action_text = "descarga" if process_command['auto_download'] else "listado"
                        await message.respond(
                            f"🔍 Iniciando {action_text} de archivos grandes...\n"
                            f"🏷️ Grupo: {process_command['group_id']}\n"
                            f"📅 Últimos {process_command['days_back']} días\n"
                            f"📊 Límite: {process_command['limit'] or 'Sin límite'}\n\n"
                            f"ℹ️ Recibirás actualizaciones del progreso."
                        )
            ...
            await message.respond(f"❌ Error ejecutando comando: {e}")
